...
export const PlotItemGroups = ({ cluster, changeCluster, currGroupID, setGroupID, isDisplayingItemGroups, setIsDisplayingItemGroups }) => {
	const {
		...
		changeItemGroupName,
		plotItemGroupsRef,
	} = PlotItemGroupsLogic({ currGroupID, cluster, changeCluster, setGroupID, setIsDisplayingItemGroups });

	if (cluster?.isAll) return null;
	return (
		<div
			ref={plotItemGroupsRef}
			className={
		...
		>
			...
			>
				<div className='unit-page-subpage-plot-item-groups'>
					{!cluster?.groups ? null : (
