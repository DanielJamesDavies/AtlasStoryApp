...
export const SettingsColour = () => {
	const { isAuthorizedToEdit, group, changeColour, revertColour, saveColour, errors } = SettingsColourLogic();
		...
		<ContentItem hasBg={true} size='s'>
			<LabelContainer label='Colour' isInline={true}>
				<EditableContainer isAuthorizedToEdit={isAuthorizedToEdit} onRevert={revertColour} onSave={saveColour}>
					<div>
						<ColourPicker value={group?.data?.colour} onChange={changeColour} enableEdit={false} horizontalAlignment='right' />
					</div>
					<div>
						<ColourPicker
