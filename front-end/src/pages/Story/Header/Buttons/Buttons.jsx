...
export const Buttons = () => {
	...
	return (
		<div className='story-header-buttons-container'>
			...
			<IconBtn className='story-header-btn' seamless={true} size='l' icon={<FaStickyNote />} onClick={goToStoryNotes} label='Story Notes' />
			{!isAuthorizedToEdit ? null : (
				<IconBtn className='story-header-btn' seamless={true} size='l' icon={<FaCog />} onClick={openSettings} label='Story Settings' />
			)}
			{story?.data?.members.findIndex((e) => e.user_id === user_id && e.type !== "owner") === -1 ? null : (
				<IconBtn className='story-header-btn' seamless={true} size='l' icon={<FaSignOutAlt />} onClick={leaveStory} label='Leave Story' />
			)}
		</div>
	);
};
