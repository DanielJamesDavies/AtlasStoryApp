...
export const SettingsSubpages = () => {
	...
	return (
		<ContentItem hasBg={true} size='s'>
			<LabelContainer label='Subpages' className='location-subpage-settings-versions-container'>
				<EditableContainer
					...
					<div>
						{!subpages
							...
							: subpages
									...
									.map((subpage, index) => (
										<div key={index} className='location-subpage-settings-subpages-item'>
											<ContentItem hasBg={true} backgroundColour='grey3'>
												<div className='location-subpage-settings-subpages-item-name'>{subpage.name}</div>
						...
						{!subpages ? null : (
							<DragDropContainer enableDragDrop={isReorderingSubpages} onDropItem={changeSubpagesOrder}>
								{subpages
									.filter((e) => e.id !== "settings")
									.map((subpage, index) => (
										<DragDropItem key={index} index={index} className='location-subpage-settings-subpages-item'>
											<ContentItem hasBg={true} backgroundColour='grey3'>
												{subpage?.isCustom ? (
													<div className='location-subpage-settings-subpages-item-name'>
														<TextInput
															value={subpage.name}
