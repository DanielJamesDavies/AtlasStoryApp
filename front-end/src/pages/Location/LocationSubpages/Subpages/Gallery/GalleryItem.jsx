...
export const GalleryItem = ({ image, index, isEditing, removeGalleryItem, onClick }) => {
	...
	return (
		<div className='location-subpage-gallery-item'>
			{!galleryItemImage ? (
				<div className='location-subpage-gallery-item-placeholder'>
			...
			{!isEditing ? (
				<>
					{image.caption.split(" ").join("").length === 0 ? null : (
						<div className='location-subpage-gallery-item-caption'>{image.caption}</div>
					)}
				</>
			) : (
				<TextInput
					className='location-subpage-gallery-item-caption'
					seamless={true}
					autoResize={true}
					label='Caption'
					value={image.caption}
			...
			{!isEditing ? null : (
				<div className='location-subpage-gallery-item-btns-container'>
