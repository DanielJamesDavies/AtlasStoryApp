...
export const CustomItem = ({
			...
			<div
				className={
					customItem?.text.join("").split(" ").join("").length !== 0 && customItem?.text.length === 1
						? "location-subpage-custom-item location-subpage-custom-item-has-single-line-text"
						: "location-subpage-custom-item"
				}
			>
				<ContentItem hasBg={true}>
					<div className='location-subpage-custom-item-title-container'>
						<div className='location-subpage-custom-item-title'>{customItem?.title}</div>
					</div>
					{customItem?.text.join("").split(" ").join("").length === 0 ? null : (
						<div className={devItemTextContainerClassName}>
							<Text className='location-subpage-custom-item-text-text' value={customItem?.text} />
						</div>
					)}
					{!customItem?.images || customItem?.images?.length === 0 ? null : (
						<div className={devItemImagesContainerClassName}>
							<div className='location-subpage-custom-item-images'>
								{customItem.images.map((image, index) => (
									<div key={index} className='location-subpage-custom-item-image-item'>
										{!locationImages.find((e) => e._id === image.image)?.image ? (
											<div className='location-subpage-custom-item-image-item-placeholder'>
										...
										{image.caption.split(" ").join("").length === 0 ? null : (
											<div className='location-subpage-custom-item-image-item-caption'>{image.caption}</div>
										)}
									</div>
								))}
							</div>
	...
	return (
		<div className='location-subpage-custom-item'>
			<ContentItem hasBg={true}>
				<div className='location-subpage-custom-item-content'>
					<div className='location-subpage-custom-item-title-container'>
						<TextInput
							className='location-subpage-custom-item-title'
							seamless={true}
							label='Title'
					...
					<div className={devItemTextContainerClassName}>
						<MultiLineTextInput
							className='location-subpage-custom-item-text'
							seamless={true}
							label='Content'
							...
							>
								{customItem.images.map((image, imageIndex) => (
									<DragDropItem key={imageIndex} index={imageIndex} className='location-subpage-custom-item-image-item'>
										...
										/>
										<div className='location-subpage-custom-item-image-item-btns-container'>
											<IconBtn
												icon={<FaTimes />}
												iconName='remove'
												seamless={true}
				...
				<div className='location-subpage-custom-item-buttons-container'>
