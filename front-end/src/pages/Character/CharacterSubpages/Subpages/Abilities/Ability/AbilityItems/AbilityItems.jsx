...
export const AbilityItems = ({ ability, changeAbility }) => {
	...
	return (
		<EditableContainer
			className='character-subpage-abilities-ability-items-container'
		...
		>
			<div className='character-subpage-abilities-ability-items'>
				{ability?.items?.map((abilityItem, index) => (
					<div key={index} className='character-subpage-abilities-ability-item-container'>
						<AbilityItem ability={ability} changeAbility={changeAbility} abilityItem={abilityItem} index={index} isEditing={false} />
					</div>
				))}
			</div>
			<DragDropContainer
				className='character-subpage-abilities-ability-items'
				enableDragDrop={isReorderingAbilityItems}
				onDropItem={reorderAbilityItems}
			>
				{ability?.items?.map((abilityItem, index) => (
					<DragDropItem key={index} index={index} className='character-subpage-abilities-ability-item-container'>
