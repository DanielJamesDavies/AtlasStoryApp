...
export const DevelopmentItemLogic = ({ developmentItem, index, characterImagesCurrDevItemIndex, isEditing }) => {
	...
	useEffect(() => {
		function getDevItemTextContainerClassName() {
			...
			if (developmentItem?.images?.length === 0) newClassName += " character-subpage-development-item-text-container-full-width";
		...
		function getDevItemImagesContainerClassName() {
			...
			if (developmentItem?.images?.length === 0) newClassName += " character-subpage-development-item-images-container-no-width";
			if (developmentItem?.value?.join("").split(" ").join("").length === 0)
