...
module.exports = async (req, res) => {
	...
	let following = await UserFollow.find({ follower_id: user_id })
		.exec()
		.catch(() => false);
	...
	following = await Promise.all(
		following.map(async (userFollow) => {
			const authUserBlock = await UserBlock.findOne({ blocked_user_id: auth_user_id, user_id: userFollow?.following_id })
				.exec()
				.catch(() => false);
			if (authUserBlock?._id) return false;

			const user = await User.findById(userFollow?.following_id)
				.exec()
				.catch(() => false);

			if (user?.isPrivate && JSON.stringify(user?._id) !== JSON.stringify(auth_user_id)) {
				const authUserFollow = await UserFollow.findOne({ following_id: userFollow?.following_id, follower_id: auth_user_id })
					.exec()
					.catch(() => false);
	...
	return res.status(200).send({ message: "Success", data: { following } });
};
